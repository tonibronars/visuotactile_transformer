...
tactile_only = False
vision_only = True
model_name = 'vision_resnet_classification_erase_big' 
...
'''
visuotactile_model = torch.load(os.environ['HOME'] + '/src/visuotactile_transformer/models/{}/visuotactile_model_best.pth'.format(model_name),map_location='cpu')
...
def process_img(path, vision_transforms, transforms, model, device):
    ...
    with torch.no_grad():
        ...
        feat = Variable(feat)
        if tactile_only: output  = model(torch.unsqueeze(feat,0))
        elif vision_only: output  = model(torch.unsqueeze(vision_feat,0))
        else: output  = model(torch.unsqueeze(vision_feat,0), torch.unsqueeze(feat,0))
        return output
...
for i, ls in enumerate(list_shapes):
    print(ls)
    output = process_img(ls, vision_transforms, transforms, visuotactile_model, device)
